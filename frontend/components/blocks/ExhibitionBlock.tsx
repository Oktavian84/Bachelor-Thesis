...
export function ExhibitionBlock({ exhibition }: Readonly<ExhibitionBlockProps>) {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof ExhibitionFormData, string>>
  >({});
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Failed to submit booking: ${response.status}`
        );
      }
  ...
  return (
    <section className="w-full flex items-center pt-28 xl:pt-35 bg-white text-black dark:bg-black dark:text-white">
      <div className="w-full flex flex-col xl:flex-row xl:items-center">
        <div className="w-full xl:w-[45%] h-auto xl:h-[65vh] xl:px-16 xl:ml-10 flex items-center">
          <div className="p-8 xl:p-12 w-full flex flex-col justify-center">
            <h2 className="font-caudex text-center text-4xl font-bold mb-8">
              {exhibition.title}
            </h2>
            ...
            {(exhibition.date || exhibition.time) && (
              <div className="md:flex justify-evenly mb-8 text-center">
                {exhibition.date && (
                  <p className="font-caudex text-xl mb-2 md:mb-0">
                ...
                {exhibition.time && (
                  <p className="font-caudex text-xl">
                    <strong>Time:</strong>{" "}
                    {formatTime(exhibition.time, exhibition.endTime)}
                  </p>
                )}
              </div>
            )}

            <div className="font-caudex text-base md:text-xl text-center leading-relaxed mb-6 whitespace-pre-line">
            ...
            <button
              ...
              className={`font-caudex px-20 py-2 rounded-lg text-xl font-bold border hover:scale-110 shadow-sm self-center transition-all duration-300 ease-in-out bg-black text-white border-black hover:bg-white hover:text-black hover:border-white shadow-black dark:border-white dark:shadow-white ${
                showForm ? "invisible pointer-events-none" : "visible"
              }`}
            >
              Book
            </button>
        ...
        <div className="w-[95%] xl:w-[45%] h-[65vh] ml-auto mt-8 mb-12 xl:mt-0 pb-8 xl:pb-0">
            ...
            <div className="relative w-full h-full rounded-tl-[8rem] rounded-bl-[8rem] overflow-hidden">
              ...
              {submitStatus === "success" && (
                <div className="absolute top-0 left-10 md:left-0 right-0 p-6 text-center">
                  <p className="font-caudex text-2xl lg:text-3xl text-black dark:text-white">
            ...
            <div className="relative rounded-tl-[8rem] rounded-bl-[8rem] p-12 h-full flex flex-col justify-center overflow-hidden">
              ...
              >
                ...
                <div className="max-w-fit mx-auto">
                  <p className="font-caudex text-center text-2xl lg:text-3xl mb-6 md:mb-20 -mt-4 text-black dark:text-white">
                  ...
                  <div>
                    <label
                      htmlFor="name"
                      className="block font-caudex text-xl sm:text-2xl mb-2 text-black dark:text-white"
                    >
                      Name
                    </label>
                    <FormInput
