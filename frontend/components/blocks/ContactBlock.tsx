...
export function ContactBlock({
  ...
  return (
    <section className={`w-full flex items-center pt-30 ${isLight ? 'bg-white' : 'bg-black'}`}>
      <div className="w-full flex flex-col xl:flex-row">
            ...
            <div className="relative w-full h-full rounded-tr-[8rem] rounded-br-[8rem] overflow-hidden">
              ...
              {submitStatus === "success" && (
                <div className="absolute top-0 right-10 xl:left-10 md:left-0 md:right-0 p-6 text-center">
                  <p className={`font-caudex text-2xl lg:text-4xl ${isLight ? 'text-black' : 'text-white text-shadow-lg/30'}`}>
            ...
            <div className="relative rounded-tr-[8rem] rounded-br-[8rem] p-12 h-full flex flex-col justify-center overflow-hidden">
              ...
              >
                ...
                <div className="max-w-fit mx-auto">
                  <p className={`font-caudex text-center text-2xl lg:text-3xl mb-10 md:mb-15 mt-4 ${isLight ? 'text-black' : 'text-white text-shadow-lg/30'}`}>
                    Please fill in your information and a contact message.
                  </p>
                  <div>
                    <label
                      htmlFor="name"
                      className={`block font-caudex text-xl mb-2 ${isLight ? 'text-black' : 'text-white text-shadow-lg/30'}`}
                    >
                      Name
                    </label>
                    <FormInput
        ...
        <div className="w-full xl:w-[35%] -mt-20 mb-20 xl:mb-0 xl:mt-0 ml-auto order-1 xl:order-2">
          <div className="p-6 xl:p-12 h-[75vh] flex flex-col justify-center items-center xl:items-start">
            <h2 className={`font-caudex text-4xl font-bold mb-8 text-center xl:text-left ${isLight ? 'text-black' : 'text-white'}`}>
              {headline}
            </h2>
            <div className={`font-caudex text-2xl sm:text-3xl leading-relaxed mb-14 whitespace-pre-line text-center xl:text-left ${isLight ? 'text-black' : 'text-white'}`}>
            ...
            <div className="flex flex-col items-center xl:items-start mb-12">
                ...
                <div className="flex gap-4 mb-8 sm:mb-12">
                  {cta.map((link, index) => (
                    <Link
                      ...
                      aria-label={
                        link.text || (index === 0 ? "Facebook" : "Instagram")
                      }
                    >
                      {index === 0 ? (
                        ...
                        <span className={`text-sm ${isLight ? 'text-black' : 'text-white'}`}>{link.text}</span>
                      )}
                    </Link>
                  ))}
                </div>
              ...
              <button
                ...
                className={`font-caudex px-20 py-2 rounded-lg text-xl font-bold border hover:scale-110 shadow-sm transition-all duration-300 ease-in-out ${
                  isLight
